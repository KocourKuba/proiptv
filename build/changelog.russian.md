### Версия 8.2.1800
- Улучшен поиск EPG для Интернет источников
- Источники XMLTV из плейлиста автоматически выбираются при первом добавлении
- Выбранные источники XMLTV не всегда проверялись и индексировались при запуске плагина

### Версия 8.2.1792
- Переделана работа с Интернет источниками.
- Добавлена возможность использовать Интернет источники не только с шаблонами, но и обычными плейлистами. Но тут все будет зависеть от правильности составленного плейлиста.
- Изменен вид настроек Интернет источников EPG. Теперь он похож на экран настроек XMLTV. В нем можно выбрать используемые источники и их приоритет.
- Добавлена возможность проверить соответствия плейлисту и XMLTV на предмет наполнения EPG для каналов плейлиста.
- Такая же возможность есть для Интернет источников моего сервера EPG (proiptv)
- Добавлен комбинированный режим EPG: Интернет источники + XMLTV. Поиск EPG проводится сначала в Интернет источника, а затем в XMLTV. В моем сервере EPG исходными данными являются оригинальные XMLTV, так что добавлять их "на всякий случай" не имеет смысла.
- При просмотре EPG из списка каналов (НЕ БРАУЗЕРА!) показывается где этот EPG был найден.
- Добавлен разбор описания телепередач в EPG для показа в ExtEPG
- Как всегда не забывайте про резервные копии!

### Версия 8.1.1752
- Из плагина полностью убрано использование источников drm-play
- Добавлены Интернет источники для провайдеров которых не было в drm. Теперь все шаблоны имеют свой Интернет источник EPG
- Плагин использует новые возможности моего конвертера xmltv -> json на EPG сервере.
- Добавлен расширенный поиск EPG для Интернет источника, такой же как реализован для XMLTV. Если EPG ID указанный в плейлисте не найден, то будет произведен поиск по имени канала. Тем самым частично исправляются огрехи составителей плейлистов.
- Оптимизированы запросы к моему Интернет серверу EPG. Плагин проверяет есть ли вообще указанные EPG ID на сервере и если их нет, то запрос к серверу не производится. Тем самым снижается бесполезная нагрузка на сервер.
- Новый конвертер так же предоставляет дополнительную информацию извлекаемую из XMLTV, так что теперь возможно использовать Ext EPG (расширенное EPG) и для Интернет источника. Но все зависит от исходного XMLTV
- Улучшено кэширование для EPG, для снижения нагрузки на сервер.

### Версия 8.0.1744
- В связи с проблемами drm-play добавлен собственный EPG сервер для ограниченного количества источников XMLTV
- Добавлен Интернет источник EPG для iptvx.one
- Информация о потоках канала вынесена в отдельное диалоговое окно
- Не работала сортировка списка плейлистов
- Полностью отключен запуск плагина при отключенном возобновлении просмотра.
- Исправлена ошибка обновления выбранных Интернет EPG источников

### Версия 8.0.1736
- Добавлена проверка для Избранных каналов если канал был удален или изменился его ID

### Версия 8.0.1734
- Немного подправлен размер шрифта в браузере каналов для категорий/каналов
- Добавлен пункт меню сброса иконки категории по умолчанию
- Исправлена ошибка обработки общего Избранного
- Добавлена "справка" для редактирования каналов/категорий
- Медиатека для Yosso переведена на стандартный M3U. Не лучший вариант - но хоть что-то
- Добавлена поддержка атрибута tvg-description для медиатеки Yosso. Но Yosso убрал этот атрибут в угоду криворуким разрабам ведроид плееров
- Исправлено контекстное меню для списка проигрывания m3u VOD плейлистов
- Если ссылка в плейлисте провайдера не совпадает с шаблонной - ID канала автоматически переключается на хэш ссылки. (Привет "заимствованным у Wink" ссылкам региональных каналов Yosso)
- При сбросе User-Agent изменения вступали в силу только после перезагрузки плагина
- Исправлено падение плагина пр добавления нового шаблона Sharaclub
- Исправлена ошибка при создании списка Интернет источника, которая приводила к невозможности пользоваться Интернет источником для нового шаблона IPTV провайдера.
- Поправлен перевод

### Версия 8.0.1718
- Исправлена ошибка выбора начального экрана если стартовым выбран список плейлистов
- Выбор плейлиста убран из контекстного меню экрана категорий если стартовым выбран список плейлистов
- Настройка вид экрана списка плейлистов теперь сохраняется в настройках плагина, а не плейлиста. 
- Исправлена ошибка показа донатных QR кодов
- Кнопка CLEAR в списке XMLTV источников теперь просто очищает кэш. А не удаляет источник. Удаление только через меню. Для безопасности.
- Исправлена ошибка запроса для показа избранных каналов.
- Добавлена настройка для полного отключения поддержки Медиатеки для выбранного провайдера.
- Убрана поддержка Медиатеки для Yosso (они не смогли).

### Версия 8.0.1706
- Для последних прошивок r24 доступна возможность отложенной загрузки иконок из XMLTV. Плагин запускается быстрее, иконки показываются после того как обработка XMLTV будет окончена
- Уменьшено потребление памяти при разборе M3U плейлиста
- Добавлены готовые ссылки для популярных XMLTV источников
- Добавлена настройка максимальной доступной памяти для плагина (для больших плейлистов)
- Добавлена настройка включения/отключения показа количества каналов/скрытых в информации о категории (так же немного ускоряет загрузку плагина при больших плейлистах). По умолчанию отключено
- Изменено назначение цветных кнопок на экранах категории/каналов
- Добавлены экраны редактирования списков категорий/каналов. Перемещение, переименование, назначение иконки перенесено в них
- Добавлена возможность перемещения группы категорий/каналов. Выделение по кнопке Enter/Ok. Очистка всех выделенных - CLEAR. 3 режима перемещения категорий/каналов по кнопкам LEFT/RIGHT - по шагам, на страницу, начало/конец
- Добавлена возможность переименовать категорию/канал
- Добавлена возможность посмотреть текущие ссылки на плейлист/медиатеку для шаблонов провайдера (кнопка Info на плейлисте)
- Добавлена возможность запуска плагина с экрана списка плейлистов, а не категорий каналов текущего плейлиста
- Добавлена возможность выбора последней использованной папки при изменении иконок категорий/обоев
- Некоторые настройки перенесены из памяти приставки в основную базу данных и теперь будут восстанавливаются из резервной копии
- Исправлена ошибка при которой плагин забывал порядок Интернет источников EPG при выходе
- Для провайдера RU TV добавлен дополнительный плейлист медиатеки
- Обновлены конфигурации Sharavoz/Uspeh/TV Team
- Добавлены отсутствующие пункты меню для NewUI
- Обновлен перевод

### Версия 7.1.1660
- Обновлен API Sharaclub
- Добавлена поддержка фильтра по году для Cbilling/Antifriz
- Небольшие исправления расположения элементов в диалоге показа EPG
- Обновлены ссылки на плейлист для CBilling
- Изменен механизм выбора аудио потоков в VOD (через контекстное меню) для провайдеров которые это поддерживают
- Добавлена возможность выбора аудио потока для Mir Kino
- На экране выбора эпизода/фильма показывается информация о доступных вариантах качества и аудио потоков для него
- Исправлена ошибка загрузки списка фильмов в VOD для IPTV Online
- Исправлена ошибка сортировки Интернет EPG источников
- Исправлена ошибка передачи ссылки для внешнего плеера
- Исправления и дополнения для диалога информации о канале/VOD
- Оставшиеся проблемы. В Mir Kino сервер не всегда выдает видео поток 1080p если он выбран по умолчанию (возможно до вас никто его не запрашивал). Как вариант - переключиться на 2060p и начать проигрывание или для не ATV приставок запросить информацию о фильме по кнопке Info. После этого переключиться обратно на 1080p и все будет работать и в дальнейшем. Проблема однозначно на стороне сервера и их админам было это сообщено. Пока не исправят - танцуйте с бубном. ;)  

### Версия 7.1.1640
- Добавлена поддержка провайдера VOD Mir Kino (https://mir-kino.pp.ru/) (Бывший BCU Media VOD)
- Добавлен внешний ffmpeg для корректного определения информации о потоке. Поэтому плагин стал "пожирней". У системного проблемы с https ссылками. Как только добавят новую версию в прошивку - я его уберу. 
- Добавлена возможность посмотреть информацию о потоке для VOD
- Улучшено отображение в окне информации о канале/VOD
- Расширен диапазон кэширования интернет источника EPG до 7 дней. Например drm-play выдает всю инфу на несколько дней, т.е. всю из XMLTV. Поэтому проверять и дергать сервер можно реже.
- Добавлен новый сервер плейлиста для Yosso
- Изменена ссылка плейлиста для провайдера IPStream
- Убраны Интернет источники EPG для Uspeh и IPTV Online (drm-play их больше не поддерживает)
- Обновлена конфигурация для ITV Live
- Увеличен таймаут для отправки лога разработчику (нынешний мой сервер чуточку тормозной)
- Убрано определение расширения для потока Sharavoz VOD (Сервер настроен некорректно и у внешних плееров едет крыша)

### Версия 7.1.1620
- Исправлено падение плагина при вызове из системного раздела настроек
- Добавлена поддержка нескольких плейлистов Медиатеки (на будущее)
- Настройка выбора Интернет источников перенесена в базу настроек плагина (теперь сохраняется в бэкапе)
- Дополнительная информация о типе Медиатеки (Server/M3U)
- Обновлен перевод
- Убраны логотипы провайдеров более не поддерживаемых плагином
- Конфигурация: Новые домены для Glanz
- Конфигурация: Для Uspeh добавлены зеркала плейлистов EU/RU (вместо переключения сервера как было раньше)
- Конфигурация: Добавлен провайдер Allway TV (https://allway.tv/) - нестандартный клон Edem, но без VPortal
- Конфигурация: Добавлен провайдер IPTV Cinema (https://iptvcinema.ru/)

### Версия 7.1.1596
- Окно настроек интерфейса плагина не показывалось
- Дополнительная проверка на путь к иконке указывающий на локальный файл 

### Версия 7.1.1594
- Добавлен перевод на Chinese Simplified
- Переделано окно настроек плейлиста
- Исправлен некорректный показ цветных кнопок при выборе файла/папки
- Добавлена возможность отключения поддержки NewUI
- Добавлена возможность изменения Интернет источников
- Добавлена обработка выбора папок содержащих символы []{}
- Добавлена возможность выбора сервера вещания для Vivamax
- Добавлена возможность выбора домена плейлиста для SatQ
- Добавлен фильтр по стране для Sharaclub/IpStream/Glanz
- Уточнения для медиатеки IpStream/iShara
- Добавлена возможность выбрать VOD медиатеку BCU в качестве отдельного провайдера. Просто для теста, претензии не принимаются, ибо сама реализация у них хромает на все четыре ноги и голову.
- Добавлена возможность менять приоритет/отключение Интернет источников EPG если их больше 1. В настройках плейлиста.

### Версия 7.1.1576
- Настройки плагина управлением скачивания перенесены в отдельный экран настроек
- Кэш запросов принудительно очищается при обновлении плагина
- Добавлена кнопка для очистки кэша запросов
- Добавлена проверка некоторых параметров плагина на корректность

### Версия 7.1.1574
- Добавлен еще один вариант выбора иконки каналов XMLTV + Плейлист
- Некоторые настройки провайдера отключаются если выбрана прямая ссылка на плейлист или файл
- Исправлены ошибки получения информации об аккаунте в CBilling, TVTeam, TVClub
- Исправлена ошибка выбора иконки канала из XMLTV источника
- Исправлено некорректное сообщение об ошибке при импорте файла с ссылками

### Версия 7.1.1564
- Неправильно формировалась ссылка для фильмов в медиатеке CBilling

### Версия 7.1.1562
- В диалог информации о плагине добавлена возможность посмотреть текущий EPG (тот, что вызывается по кнопке SUBTITLE)
- Возобновление просмотра при запуске плагина принудительно прерывается если перед этим был изменен плейлист
- Добавлен костыль для ссылок на иконки в плейлисте провайдера 1ott (ребята из 1ott, протокол таки надо указывать!)
- Добавлен новый кэш запросов сервера. В основном применим к VOD. Добавлен из за особенностей API JellyFin
- Добавлена дополнительная настройка API Domain (для некоторых провайдеров это критично, потому что сервер обработки команд и сервер для скачивания плейлистов разный, например TV Team и его клоны)
- Добавлена медиатека Yosso. Внедрена поддержка JellyFin API
- Дополнение к обработке XMLTV, те кто пишет генераторы XMLTV в очередной раз пробили дно. Ну может таки начнем читать доки?
- Дополнительные проверки на удачную распаковку XMLTV
- Исправлен показ категории Измененные каналы в NewUI (старые и неактуальные настройки могли на это влиять и плагин не показывал эту категорию)
- Исправлена обработка дополнительных данных для Ext_Epg. Если информация которую поддерживает Ext_Epg есть в XMLTV или Интернет источнике EPG, то она передается плагину Ext_Epg
- Исправлена ошибка индексирования XMLTV источников если их выбрано несколько
- Исправлена ошибка обновления NewUI после перезагрузки приставки
- Исправлена ошибка чистой установки
- Исправлены некоторые ошибки при выборе папки/файла
- Исправлены ошибки при обновлении/очистке кэша XMLTV/Интернет источник 
- На функцию добавления в Избранное, там где это используется, навешана кнопка FAV1 (новые пульты)
- На функцию удаления, в там где это используется, так же навешана кнопка CLEAR (у кого есть - пользуйтесь)
- Обновлена конфигурация для Shura TV, TVTeam, Sharaclub
- Переделана обработка ETAG (http заголовок запроса If-None-Match) для более стабильного результата получения информации и ошибочного стирания существующего кэша EPG в случае проблем со связью с сервером. Не получилось проверить - значит ничего не меняем.
- Пересмотрена привязка цветных кнопок на некоторых экранах плагина
- Плагин принудительно проверяет актуальность всех выбранных источников XMLTV, если выбран соответствующий движок XMLTV
- Показ заголовка плагина (с указанием текушего плейлиста) теперь одинаков и в верхнем меню и в popup
- Убрана возможность выбирать набор Интернет источника EPG. Теперь плагин просто пробует найти EPG во всех доступных источниках прописанных в конфигурации провайдера. Чуть дольше, но за то надежней. Плюс помогает решить проблему с несколькими источниками EPG в плейлисте по типу TV Boom
- Улучшен алгоритм загрузки каналов, теперь плагин реже запрашивает/обновляет плейлист после выполнения определенных команд
- Улучшена обработка ошибок индексирования XMLTV. В случае ошибок плагин старается показать сообщение об ошибке (к сожалению показ сообщения об ошибке иногда невозможен из-за ограничений API Dune)
- Уменьшено количество запросов к серверу API для получения информации о подписке/серверах/доменах etc

### Версия 7.0.1478
- Добавлена настройка для запрета таймера отключения
- Добавлена настройка способа отключения по таймеру. Как в системе или отключения видеовыхода
- Возвращена возможность изменить сервер вещания для TV Team
- Возвращен показ текущего плейлиста в верхнем меню. Пришлось очень постараться
- Пересмотрена обработка кнопок в браузере просмотра. Все цветные кнопки в этом режиме работают как задано движком Dune
- Переделана обработка возобновления просмотра для корректной работы в NewUI и Classic
- Исправлена обработка некоторые ошибок в NewUI
- Исправлены строки перевода для некоторых пунктов контекстного меню NewUI

### Версия 7.0.1460
- Запрет запуска плагина если приставка пытается возобновить просмотр, но в настройках плагина он отключен
- Таймер отключения не срабатывал на некоторых приставках

### Версия 7.0.1452
- Добавлена возможность использовать несколько изображений передачи из XMLTV (если есть) в Ext EPG
- Подстройка начала дня в зависимости от часового пояса (рекомендации DuneHD)
- Добавлена настройка изменения размера шрифта для групп/каналов в окне в режиме просмотра передачи
- Добавлено предупреждение при выборе движка XMLTV об отсутствии выбранных источников
- Возобновление просмотра теперь доступно и для Медиатеки. Т.е. вместо ТВ канала будет возобновляться фильм если выключение было во время просмотра
- Добавлен таймер отключения при просмотре передачи или фильма. НЕДОСТУПНО для ATV приставок (на основе Homatics)!
- Желтая кнопка в режиме просмотра вызов окна с фиксированным выбором времени отключения.
- Зеленая кнопка в режиме просмотра - ручной режим таймера. Шаг можно изменить в настройках плагина, по умолчанию 1 минута.
- Красная кнопка в режиме просмотра - отключение таймера. Так же можно отключить выбрав "Нет" в качестве времени отключения в окне вызываемом по желтой кнопке.
- За 2 минуты до отключения показывается OSD с обратным отсчетом, время показа обратного отсчета так же можно задать в настройках плагина
- При выходе из режима просмотра передачи/фильма таймер отключается!
- Новый механизм оповещения плагина об окончании индексации XMLTV, теперь плагин не проверяет каждую секунду состояние процесса и не тратит на это ресурсы. НЕДОСТУПНО для ATV приставок (на основе Homatics)!
- Исправлена ошибка в окне подписки Sharaclub
- Исправлено некорректное сообщение об ошибке плагина на странице настроек EPG
- Исправлена ошибка вызова внутреннего таймера в NewUI
- Исправлена ошибка не позволяющая выбрать ссылку или файл плейлиста провайдера
- Конфигурация: добавлена возможность поменять домен для API провайдеров IPTVOnline/Tvizi (для предыдущих версий 7.x возможно понадобится зайти в настройки провайдера и выбрать нужный домен)
- Конфигурация: добавлен провайдер VivaMax
- Конфигурация: Обновлена конфигурация для Peak TV

### Версия 7.0.1422
- Исправлена ошибка приводящая к падению плагина в случае показа сообщения об ошибке
- Исправлена ошибка приводящая к падению плагина в при вызове диалога ввода пароля настроек
- Убрана поддержка на уровне web-сервера кэширования плейлистов. У "сокращателей" ссылок с ним проблемы 
