...
abstract class DunePluginFw
{
    ...
    public function invoke_operation($plugin, $call_ctx)
    {
        ...
        if ($call_ctx->op_type_code === PLUGIN_OP_GET_VOD_STREAM_INFO)
        {
            if (!method_exists($plugin, 'get_vod_stream_info'))
                throw new Exception("Method not found: get_vod_stream_info");
            return $plugin->get_vod_stream_info(
                $call_ctx->input_data, $call_ctx->plugin_cookies);
        }
    ...
    public function get_out_type_code($op_code)
    {
        ...
        if (is_null($map))
        {
            ...
            (
                ...
                PLUGIN_OP_GET_VOD_STREAM_INFO       => PLUGIN_OUT_DATA_PLUGIN_STREAM_INFO,
